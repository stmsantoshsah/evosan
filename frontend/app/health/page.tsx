...
export default function HealthPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (data.workout) {
        // CASE A: User ne aaj workout log kar diya hai -> Show logged data
        setWorkout({ ...workout, ...data.workout });
      } else {
        // CASE B: Aaj ka log khali hai -> Fetch Plan from Planner
        const dayName = new Date().toLocaleDateString('en-US', { weekday: 'long' }); // e.g. "Monday"

        try {
          const planRes = await fetch(`${API_URL}/health/plan/${dayName}`);
