...
export default function HabitsPage() {
  ...
  const fetchHabits = async () => {
    try {
      //   const res = await fetch(`${API_URL}/habits?date_str=${today}`);
      const res = await fetch(`${API_URL}/habits/?date_str=${today}`);
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <form onSubmit={createHabit} className="flex flex-col md:flex-row gap-4 mb-10 bg-zinc-900/50 p-4 rounded-lg border border-zinc-800">
        <input
          ...
          className="flex-1 bg-zinc-950 border border-zinc-800 rounded px-4 py-2 focus:border-cyan-500 outline-none"
          value={newHabitName}
        ...
        <select
          ...
          className="bg-zinc-950 border border-zinc-800 rounded px-4 py-2 text-sm outline-none"
        >
