...
export async function POST(req: NextRequest) {
  try {
    ...
    if (extractedData.water_ml || extractedData.calories || extractedData.protein_g) {
      ...
      promises.push(
        db.collection('nutrition').updateOne({ date: today }, updateDoc, { upsert: true })
      );
    }
    ...
    if (extractedData.workout_name) {
      ...
      promises.push(
        db.collection('workouts').updateOne({ date: today }, workoutUpdate, { upsert: true })
      );
    }
      ...
      if (existingJournal) {
        promises.push(
          db
            ...
            .updateOne({ _id: existingJournal._id }, { $set: { mood: extractedData.mood_rating } })
        );
      } else {
        promises.push(
