  ...
  if (error || !data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-black text-red-500 font-mono p-4">
        ...
        <Link
          href="/"
          className="mt-8 px-6 py-2 bg-zinc-900 border border-zinc-800 rounded hover:bg-zinc-800 transition-colors text-zinc-400"
        >
          Return to Home Base
        </Link>
      </div>
      ...
      <div className="max-w-6xl mx-auto mb-8 flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-bold text-white mb-1">
            EVOSAN // <span className="text-teal-500">PUBLIC_ACCESS</span>
          </h1>
          <p className="text-xs text-zinc-500 font-mono">READ_ONLY MODE ENABLED</p>
        </div>
        <Link
          href="/signup"
          className="text-xs text-teal-500 hover:text-teal-400 font-bold uppercase tracking-widest border border-teal-500/20 px-4 py-2 rounded bg-teal-500/10"
        >
          Build Your System
        </Link>
      </div>

      <ProfileView profile={data.profile} gamification={data.gamification} isEditable={false} />
    </div>
  );
}
