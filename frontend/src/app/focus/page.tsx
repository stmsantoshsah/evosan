...
export default function FocusPage() {
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center relative overflow-hidden text-gray-300 font-mono">
      {/* Background Pulse */}
      {isActive && (
        <div className="absolute inset-0 bg-teal-900/10 animate-pulse pointer-events-none" />
      )}
      ...
      <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-center z-10">
        <Link
          href="/"
          className="hover:text-white flex items-center gap-2 text-sm transition-colors"
        >
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={toggleAudio}
            className={`flex items-center gap-2 px-3 py-1 rounded border transition-all ${audioEnabled ? 'bg-teal-900/30 border-teal-500 text-teal-400' : 'border-gray-800 text-gray-500'}`}
          >
      ...
      <div className="z-10 text-center space-y-8 max-w-2xl w-full px-4">
        ...
        <div className="relative">
          ...
          />
          {isActive && (
            <div className="text-teal-500 text-xs mt-2 animate-pulse">
              SYSTEM LOCKED • RECORDING XP
            </div>
          )}
        </div>
        ...
        {/* Timer Display */}
        <div
          className={`text-[120px] md:text-[180px] font-bold leading-none tracking-tighter transition-all ${isActive ? 'text-white drop-shadow-[0_0_15px_rgba(20,184,166,0.5)]' : 'text-gray-800'}`}
        >
          {formatTime(timeLeft)}
        </div>
      ...
      <div className="absolute bottom-12 text-gray-700 text-xs flex gap-8">
        <div>
          MODULE: <span className="text-teal-600">BROWN_NOISE_GEN_V1</span>
        </div>
        <div>
          STATUS:{' '}
          <span className={isActive ? 'text-green-500' : 'text-yellow-600'}>
