        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="flex bg-zinc-900/50 p-1 rounded-xl border border-zinc-800">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold transition-all ${mode === 'text' ? 'bg-zinc-800 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
              <Type size={14} /> Text Log
            </button>
            <button
              onClick={() => setMode('voice')}
              className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold transition-all ${mode === 'voice' ? 'bg-zinc-800 text-purple-400 shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
          ...
          {isFetchingEntries ? (
            <div className="space-y-4 animate-pulse">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-32 bg-zinc-900/50 rounded-xl border border-zinc-800"
                ></div>
              ))}
            </div>
          ) : (
