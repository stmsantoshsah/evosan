            ...
            {entry.tags && entry.tags.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-zinc-800/50">
                {entry.tags.map((tag, i) => (
                  <span
                    key={i}
                    className="flex items-center gap-1 text-[10px] uppercase tracking-wider bg-black border border-zinc-800 text-zinc-500 px-2 py-1 rounded hover:text-teal-400 hover:border-teal-500/30 transition-colors cursor-default"
                  >
                    <Hash size={10} /> {tag}
                  </span>
                ))}
