...
const TabButton = ({ active, onClick, label, icon }: any) => (
  ...
  >
    {icon} {label}
    {active && (
      <span className="absolute bottom-[-1px] left-0 w-full h-[2px] bg-teal-400 shadow-[0_0_10px_#2dd4bf]" />
    )}
  </button>
);
...
const SettingCard = ({ title, children }: any) => (
  <div className="p-6 bg-zinc-900/50 border border-zinc-800 rounded-xl">
    <h3 className="font-bold text-zinc-200 mb-4 font-mono uppercase tracking-wider text-xs">
      {title}
    </h3>
    {children}
  </div>
          ...
          <div className="w-full md:w-1/4 flex flex-row md:flex-col gap-2 overflow-x-auto md:overflow-visible pb-2 md:pb-0">
            {DAYS.map((day) => (
              <button
                ...
                className={`p-3 text-left rounded-lg transition-all whitespace-nowrap ${
                  selectedDay === day
