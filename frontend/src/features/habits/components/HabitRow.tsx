      ...
      <div className="flex items-center gap-4 mb-4 md:mb-0">
        ...
        >
          {completed ? (
            ...
            <div className="w-3 h-3 rounded-full bg-zinc-700 group-hover:bg-teal-500/50" />
          )}
        </div>

        <div>
          <h3
            className={`font-bold text-lg transition-all ${completed ? 'text-zinc-500 line-through' : 'text-zinc-100 group-hover:text-teal-400'}`}
          >
            {habitName}
          </h3>
          <p
            className={`text-xs font-mono flex items-center gap-1 ${streak > 0 ? 'text-orange-500' : 'text-zinc-600'}`}
          >
