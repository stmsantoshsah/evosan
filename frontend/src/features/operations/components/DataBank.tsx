        ...
        {isCreating ? (
          <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-6 animate-in fade-in zoom-in-95">
            ...
            <div className="flex justify-end gap-2">
              <button
                ...
                className="px-4 py-2 text-zinc-500 hover:text-white text-xs"
              >
                Cancel
              </button>
              <button
                onClick={handleSaveNote}
                className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded text-xs font-bold"
              >
                Save to Memory
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {isLoading ? (
              ...
              filteredArchives.map((note: any) => <NoteCard key={note._id} note={note} />)
            )}
          </div>
        )}
      </div>
  ...
  <div className="bg-zinc-900/40 border border-zinc-800 p-4 rounded-lg hover:border-purple-500/30 transition-all cursor-pointer group hover:bg-zinc-900/60 h-48 flex flex-col justify-between">
    <div>
      <div className="flex justify-between items-start mb-2">
        <h4 className="font-bold text-zinc-300 group-hover:text-purple-300 transition-colors text-sm line-clamp-1">
          {note.title}
        </h4>
      ...
      <p className="text-xs text-zinc-500 line-clamp-4 font-mono leading-relaxed">{note.content}</p>
    </div>
    <div className="flex flex-wrap gap-1 mt-3">
      {note.tags?.map((tag: string, i: number) => (
        <span
          key={i}
          className="text-[9px] px-1.5 py-0.5 bg-zinc-950 rounded border border-zinc-800 text-zinc-500 group-hover:border-purple-900/50 group-hover:text-purple-400/70 transition-colors"
        >
          #{tag}
        </span>
      ))}
