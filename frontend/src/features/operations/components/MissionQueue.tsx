...
} from '../slices/operationsApiSlice';
      ...
      <div className="flex text-xs border-b border-zinc-800 font-mono">
        {['today', 'tomorrow', 'backlog'].map((tab) => (
          <button
            ...
            className={`flex-1 py-3 transition-colors uppercase ${
              activeTab === tab
        ...
        {isLoading ? (
          ...
          filteredMissions.map((task: any) => (
            <div
              ...
              className={`flex items-start gap-3 p-3 rounded border transition-all cursor-pointer group select-none ${
                task.isCompleted
            ...
            >
              <div className="mt-0.5">
                {task.isCompleted ? (
                  ...
                  <Circle
                    size={16}
                    className="text-zinc-600 group-hover:text-teal-500/50 transition-colors"
                  />
                )}
              </div>
              <span
                className={`text-sm font-light ${task.isCompleted ? 'line-through text-zinc-600' : 'text-zinc-200'}`}
              >
                {task.title}
              </span>
            </div>
