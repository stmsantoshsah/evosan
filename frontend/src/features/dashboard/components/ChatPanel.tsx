...
import { Send, Mic, MicOff, Volume2, VolumeX, X, Compass } from 'lucide-react';
...
export default function ChatPanel({ onClose }: ChatPanelProps) {
  ...
  const formatBoldText = (textStr: string) => {
    ...
    while ((match = boldRegex.exec(textStr)) !== null) {
      ...
      parts.push(
        <strong
          key={match.index}
          className="font-bold text-emerald-400 drop-shadow-[0_0_8px_rgba(16,185,129,0.25)]"
        >
          {match[1]}
        </strong>
      );
          ...
          if (trimmed.startsWith('* ') || trimmed.startsWith('- ') || trimmed.startsWith('• ')) {
            ...
            return (
              <div key={idx} className="flex flex-row items-start gap-2 pl-2 w-full">
                ...
                <span className="text-xs md:text-sm text-foreground/90 inline-block">
  ...
  const speakText = (text: string) => {
    ...
    utterance.rate = 0.9; // pace for verbal shadowing
    utterance.pitch = 1.0;
    ...
    try {
      ...
      if (reader) {
        while (true) {
          ...
          setMessages((prev) =>
            prev.map((msg) => (msg.id === evosanMsgId ? { ...msg, text: fullResponseText } : msg))
          );
        }
      }
      ...
      setMessages((prev) =>
        prev.map((msg) => (msg.id === evosanMsgId ? { ...msg, isStreaming: false } : msg))
      );
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        { id: userMsgId, sender: 'user', text: data.transcript || '[Audio Capture]' },
        {
          id: evosanMsgId,
          sender: 'evosan',
          text: data.response || 'Coaching connection offline.',
        },
      ]);
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground font-mono">
              GROQ LLAMA-3.3 // LANGRAPH STATEFUL
            </p>
          </div>
        </div>
      ...
      <div className="p-4 border-t border-border/40 bg-muted/20">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            placeholder={
              isRecording
                ? 'System recording vocally...'
                : 'Practice elevator introduction or ask directives...'
            }
