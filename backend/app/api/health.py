...
from app.models.health import NutritionLog, WorkoutLog, WorkoutPlan
...
async def log_workout(log: WorkoutLog):
    await db.client["evosan_db"]["workouts"].update_one(
        {"date": log.date}, {"$set": log.dict()}, upsert=True
    )
...
async def get_health_daily(date: str):
    ...
    return {
        "workout": {k: v for k, v in workout.items() if k != "_id"}
        if workout
        else None,
        "nutrition": {k: v for k, v in nutrition.items() if k != "_id"}
        if nutrition
        else None,
    }
